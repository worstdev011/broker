          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.07] shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-white">{tc('notifications')}</span>
              </div>
              {unreadCount > 0 && (
                <button
                  type="button"
            ...
            <div ref={listRef} className="overflow-y-auto flex-1 py-1 scrollbar-hide-on-idle">
              {notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 px-6 gap-2 text-center">
                  <Bell className="w-8 h-8 text-white/10" aria-hidden />
                  <span className="text-sm text-white/45 leading-snug max-w-[260px]">{tn('empty')}</span>
                </div>
              ) : (
                notifications.map((notif) => (
                  ...
                  >
                    ...
                    {!notif.read && <div className="w-1.5 h-1.5 rounded-full bg-[#3347ff] shrink-0 mt-1.5" />}
                  </button>
                ))
              )}
            </div>
